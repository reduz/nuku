{ Ported from https://github.com/cheat-engine/cheat-engine/blob/master/Cheat%20Engine/mikmod.pas
  to work on Linux with fpc 3.0.4. }
...
uses
  Classes, SysUtils, syncobjs, dynlibs; {windows}

{ Following aliases remove dependency on Windows types }
type
  size_t=DWord;
  ULONG=Longword;
  FARPROC=Pointer;
...
type
  ...
  MREADER_READ=function(self:PREADER; dest: pointer; l: size_t): Boolean; cdecl;
  MREADER_GET=function(self:PREADER):integer; cdecl;
  MREADER_EOF=function(self:PREADER):Boolean; cdecl;
...
var
  ...
  //mod player
  Player_Load: function(filename: pchar; maxchan: integer; curious: Boolean): PMODULE; cdecl;
  Player_LoadFP: function(fp: pointer; maxchan: integer; curious: Boolean): PMODULE; cdecl;
  Player_LoadGeneric: function(reader: PREADER; maxchan: integer; curious: Boolean): PMODULE; cdecl;
  ...
  Player_GetLoop: function(module: PMODULE):Boolean; cdecl;
  Player_GetWrap: function(module: PMODULE):Boolean; cdecl;
  Player_SetLoop: procedure(module: PMODULE; state: Boolean); cdecl;
  Player_SetWrap: procedure(module: PMODULE; state: Boolean); cdecl;

  Player_Active: function:Boolean; cdecl;
  Player_Stop: procedure; cdecl;
  Player_TogglePause: procedure; cdecl;
  Player_Paused: function:Boolean; cdecl;
  Player_NextPosition: procedure; cdecl;
  Player_PrevPosition: procedure; cdecl;
  Player_SetPosition: procedure(pos: Word); cdecl;
  Player_Muted: function(chan: BYTE):Boolean; cdecl;
  ...
  Voice_Stopped: function(v: SmallInt): Boolean; cdecl;
...
function mr_read(self:PREADER; dest: pointer; l: size_t): Boolean; cdecl;
begin
  if l>(self^.size-self^.position) then
    l:=self^.size-self^.position;

  {copymemory(dest, @self^.memory[self^.position], l);}
  move(self^.memory[self^.position], dest, l);
  inc(self^.position, l);
...
function mr_eof(self:PREADER):Boolean; cdecl;
begin
...
begin
  ...
  begin
    ...
    libmikmod:=loadlibrary('libmikmod.so.3');

    farproc(MikMod_RegisterAllDrivers):=GetProcedureAddress(libmikmod, 'MikMod_RegisterAllDrivers');
    farproc(MikMod_RegisterAllLoaders):=GetProcedureAddress(libmikmod, 'MikMod_RegisterAllLoaders');


    farproc(MikMod_InfoDriver):=GetProcedureAddress(libmikmod, 'MikMod_InfoDriver');
    farproc(MikMod_RegisterDriver):=GetProcedureAddress(libmikmod, 'MikMod_RegisterDriver');
    farproc(MikMod_DriverFromAlias):=GetProcedureAddress(libmikmod, 'MikMod_DriverFromAlias');
    farproc(MikMod_Init):=GetProcedureAddress(libmikmod, 'MikMod_Init');

    farproc(MikMod_Exit):=GetProcedureAddress(libmikmod, 'MikMod_Exit');
    farproc(MikMod_Reset):=GetProcedureAddress(libmikmod, 'MikMod_Reset');
    farproc(MikMod_SetNumVoices):=GetProcedureAddress(libmikmod, 'MikMod_SetNumVoices');
    farproc(MikMod_Active):=GetProcedureAddress(libmikmod, 'MikMod_Active');
    farproc(MikMod_EnableOutput):=GetProcedureAddress(libmikmod, 'MikMod_EnableOutput');
    farproc(MikMod_DisableOutput):=GetProcedureAddress(libmikmod, 'MikMod_DisableOutput');
    farproc(MikMod_Update):=GetProcedureAddress(libmikmod, 'MikMod_Update');
    farproc(MikMod_InitThreads):=GetProcedureAddress(libmikmod, 'MikMod_InitThreads');
    farproc(MikMod_Lock):=GetProcedureAddress(libmikmod, 'MikMod_Lock');
    farproc(MikMod_Unlock):=GetProcedureAddress(libmikmod, 'MikMod_Unlock');


    farproc(Player_Load):=GetProcedureAddress(libmikmod, 'Player_Load');
    farproc(Player_LoadFP):=GetProcedureAddress(libmikmod, 'Player_LoadFP');
    farproc(Player_LoadGeneric):=GetProcedureAddress(libmikmod, 'Player_LoadGeneric');
    farproc(Player_LoadTitle):=GetProcedureAddress(libmikmod, 'Player_LoadTitle');
    farproc(Player_LoadTitleFP):=GetProcedureAddress(libmikmod, 'Player_LoadTitleFP');
    farproc(Player_Free):=GetProcedureAddress(libmikmod, 'Player_Free');
    farproc(Player_Start):=GetProcedureAddress(libmikmod, 'Player_Start');
    farproc(Player_Active):=GetProcedureAddress(libmikmod, 'Player_Active');
    farproc(Player_Stop):=GetProcedureAddress(libmikmod, 'Player_Stop');
    farproc(Player_TogglePause):=GetProcedureAddress(libmikmod, 'Player_TogglePause');
    farproc(Player_Paused):=GetProcedureAddress(libmikmod, 'Player_Paused');
    farproc(Player_NextPosition):=GetProcedureAddress(libmikmod, 'Player_NextPosition');
    farproc(Player_PrevPosition):=GetProcedureAddress(libmikmod, 'Player_PrevPosition');
    farproc(Player_SetPosition):=GetProcedureAddress(libmikmod, 'Player_SetPosition');
    farproc(Player_Muted):=GetProcedureAddress(libmikmod, 'Player_Muted');
    farproc(Player_SetVolume):=GetProcedureAddress(libmikmod, 'Player_SetVolume');
    farproc(Player_GetModule):=GetProcedureAddress(libmikmod, 'Player_GetModule');
    farproc(Player_SetSpeed):=GetProcedureAddress(libmikmod, 'Player_SetSpeed');
    farproc(Player_SetTempo):=GetProcedureAddress(libmikmod, 'Player_SetTempo');


    farproc(Player_GetLoop):=GetProcedureAddress(libmikmod, 'Player_GetLoop');
    farproc(Player_SetLoop):=GetProcedureAddress(libmikmod, 'Player_SetLoop');
    farproc(Player_GetWrap):=GetProcedureAddress(libmikmod, 'Player_GetWrap');
    farproc(Player_SetWrap):=GetProcedureAddress(libmikmod, 'Player_SetWrap');


    farproc(Sample_Load):=GetProcedureAddress(libmikmod, 'Sample_Load');
    farproc(Sample_LoadFP):=GetProcedureAddress(libmikmod, 'Sample_LoadFP');
    farproc(Sample_LoadGeneric):=GetProcedureAddress(libmikmod, 'Sample_LoadGeneric');

    farproc(Sample_Play):=GetProcedureAddress(libmikmod, 'Sample_Play');
    farproc(Sample_Free):=GetProcedureAddress(libmikmod, 'Sample_Free');

    farproc(Voice_Stopped):=GetProcedureAddress(libmikmod, 'Voice_Stopped');
    ...
    md_volume:=GetProcedureAddress(libmikmod, 'md_volume');
    md_musicvolume:=GetProcedureAddress(libmikmod, 'md_musicvolume');
    md_sndfxvolume:=GetProcedureAddress(libmikmod, 'md_sndfxvolume');
    md_reverb:=GetProcedureAddress(libmikmod, 'md_reverb');
    md_pansep:=GetProcedureAddress(libmikmod, 'md_pansep');

    md_device:=GetProcedureAddress(libmikmod, 'md_device');
    md_mixfreq:=GetProcedureAddress(libmikmod, 'md_mixfreq');
    md_mode:=GetProcedureAddress(libmikmod, 'md_mode');

    MikMod_errno:=GetProcedureAddress(libmikmod, 'MikMod_errno');
